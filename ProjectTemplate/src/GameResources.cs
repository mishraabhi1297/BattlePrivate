...
namespace MyGame
{
	public static class GameResources
	{
		...
		{
			/*return _Fonts (font);*/
			foreach (KeyValuePair<string, Font> kvp in _Fonts) {
				if (kvp.Key == font) {
					return kvp.Value;
				}
			}
		...
		public static Bitmap GameImage (string image)
		{
			...
			foreach (KeyValuePair<string, Bitmap> kvp in _Images) {
				if (kvp.Key == image) {
					return kvp.Value;
				}
			}
		...
		{
			/*return _Music (music);*/
			foreach (KeyValuePair<string, Music> kvp in _Music) {
				if (kvp.Key == music) {
					return kvp.Value;
				}
			}
		...
		private static void FreeSounds ()
		{
			...
			foreach (KeyValuePair<string, SoundEffect> kvp in _Sounds) {
				SwinGame.FreeSoundEffect (kvp.Value);
			}
		}
